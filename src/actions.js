...
export function updateContract(
  ...
  let mutation = formatMutation(
    "updateContract",
    formatContractGQL(contract, readOnlyFields),
    clientMutationLabel
  );
  var requestedDateTime = new Date();
  return graphql(
...
export function approveContractBulk(
  contracts,
  clientMutationLabel,
  clientMutationDetails = null
) {
  let contractUuids = `contractUuids: ["${contracts
    .map((contract) => decodeId(contract.id))
    .join('","')}"]`;
  let mutation = formatMutation(
    "approveBulkContract",
    contractUuids,
  ...
  var requestedDateTime = new Date();
  contracts.forEach(
    (contract) => (contract.clientMutationId = mutation.clientMutationId)
  );
  return graphql(
    mutation.payload,
    [
    ...
    ],
    {
      clientMutationId: mutation.clientMutationId,
      clientMutationLabel,
      clientMutationDetails: !!clientMutationDetails
        ? JSON.stringify(clientMutationDetails)
        : null,
      requestedDateTime,
    }
  );
...
export function counterContractBulk(
  contracts,
  clientMutationLabel,
  clientMutationDetails = null
) {
...
export function createContractDetails(contractDetails, clientMutationLabel) {
  let mutation = formatMutation(
    ...
    formatContractDetailsGQL(contractDetails),
    clientMutationLabel
  );
  var requestedDateTime = new Date();
  return graphql(
...
export function deleteContractDetails(
  contractDetails,
  clientMutationLabel,
  clientMutationDetails = null
) {
